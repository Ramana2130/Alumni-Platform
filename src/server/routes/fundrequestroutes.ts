...
import { createFundRequest, deleteFundRequest, getAllFundRequests, getFundRequestById, updateFundRequestStatus } from "../models/fundrequestschema.js";
...
router.post("/add", async (req, res) => {
  try {
    ...
    res.status(201).json({ id, message: "Fund request submitted successfully" });
  } catch (err) {
    res.status(500).json({ error: err instanceof Error ? err.message : "Error creating fund request" });
  }
});
...
router.put("/updateStatus/:id", async (req, res) => {
  try {
    ...
    if (affectedRows === 0) return res.status(404).json({ error: "Fund request not found" });
