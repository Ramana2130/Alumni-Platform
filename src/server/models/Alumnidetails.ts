...
export async function createAlumni(data: any) {
  ...
  const [result] = (await pool.query(sql, values)) as [
...
export async function getAllAlumni() {
  const [rows] = await pool.query(
    "SELECT * FROM alumni_details ORDER BY id DESC"
  );
  return rows;
}
...
export async function getAlumniById(id: number) {
  const [rows] = await pool.query<RowDataPacket[]>(
    "SELECT * FROM alumni_details WHERE id = ?",
    [id]
  );
...
export async function deleteAlumni(id: number) {
  ...
  const [result] = await pool.query<import("mysql2").ResultSetHeader>(sql, [
    id,
  ]);
  return result.affectedRows;
}
