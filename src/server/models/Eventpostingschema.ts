...
export async function getAllEvents() {
  const [rows] = await pool.query("SELECT * FROM events_posting ORDER BY created_at DESC");
  return rows;
}

// Get event by id
export async function getEventById(id: number) {
  const [rows]: [any[], any] = await pool.query("SELECT * FROM events_posting WHERE id = ?", [id]);
  return rows[0];
}
...
export async function deleteEvent(id: number): Promise<number> {
  const [result]: [ResultSetHeader, any] = await pool.query("DELETE FROM events_posting WHERE id = ?", [id]);
  return result.affectedRows;
}
