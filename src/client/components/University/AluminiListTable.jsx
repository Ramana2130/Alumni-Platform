...
const AlumniListTable = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            setLoading(true);
            const result = await axios.get(`http://localhost:3000/alumni/${universityId}/alumnis`);
    ...
    const readUploadFile = async (e) => {
        ...
        if (e.target.files) {
            ...
            try {
                setLoading(true);
                const universityId = localStorage.getItem("_id");
                const response = await axios.post(`http://localhost:3000/alumni/addexcelfile/${universityId}`, formData, {
                    headers: {
                        ...
                        <Fragment>
                            <div className="flex items-center space-x-2 border border-[#B1D609]">
                                ...
                                <label htmlFor="input" className="text-sm text-[#cff80b]">
                                    Note: The headers in the Excel file should be as follows: Name, Department, Email, Year of Joining, Year of Passing.
                                </label>
                            </div>
                        ...
                        <div className="mt-7 h-[52vh] overflow-auto">
                            <table className="w-full whitespace-nowrap">
                                <thead className="text-center bg-gray-800">
                                    <tr>
                                        <th className="text-sm text-white uppercase py-2 px-4">Name</th>
                                        <th className="text-sm text-white uppercase py-2 px-4">Department</th>
                                        <th className="text-sm text-white uppercase py-2 px-4">Year of Joining</th>
                                        <th className="text-sm text-white uppercase py-2 px-4">Email</th>
                                        <th className="text-sm text-white uppercase py-2 px-4">Year of Passing</th>
                                        <th className="text-sm text-white uppercase py-2 px-4">Register No</th>
                                        <th className="text-sm text-white uppercase py-2 px-4">Mobile Number</th>
                                        <th className="text-sm text-white uppercase py-2 px-4">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {currentTasks.length > 0
                                            ...
                                            <tr className="h-16 border-b" key={task._id}>
                                                <td className="text-center py-2 px-4">
                                                    <div className="flex items-center justify-center">
                                                        <p className="text-base font-medium leading-none text-gray-300 uppercase">{task.alumniname}</p>
                                                    </div>
                                                </td>
                                                <td className="text-center py-2 px-4">
                                                    <div className="flex items-center justify-center">
                                                        <p className="text-base text-[#CFF80B] font-bold">{task.department}</p>
                                                    </div>
                                                </td>
                                                <td className="text-center py-2 px-4">
                                                    <input
                                                ...
                                                <td className="text-center py-2 px-4">
                                                    <p className="text-base font-bold leading-none text-white">{task.passedoutyear}</p>
                                                </td>
                                                <td className="text-center py-2 px-4">
                                                    <p className="text-base font-bold leading-none text-white">{task.alumniregisterno}</p>
                                                </td>
                                                <td className="text-center py-2 px-4">
                                                    <p className="text-base font-bold leading-none text-white">{task.alumnimobilenumber}</p>
                                                </td>
                                                <td className="text-center py-2 px-4">
                                                    <Link to={`/updatealumnidetails/${task.universityId}/${task._id}`} className="text-yellow-400 hover:text-yellow-600">
                                                    ...
                                                    <button onClick={() => handleDelete(task._id)} className="text-red-500 hover:text-red-700 ml-2">
                                        ...
                                        : <tr><td colSpan="8" className="text-center text-white">No data available</td></tr>}
                                </tbody>
                            </table>
                        </div>
                        <div className="flex items-center justify-between mt-7">
                            <button onClick={prevPage} className="px-6 py-3 bg-[#B1D609] text-black rounded-full font-bold">Previous</button>
                            <button onClick={nextPage} className="px-6 py-3 bg-[#B1D609] text-black rounded-full font-bold">Next</button>
                        </div>
                    </div>
                </div>
            </div>
