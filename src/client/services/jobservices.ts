...
export const createJobPosting = async (jobData: any) => {
  const response = await axios.post(
    `${import.meta.env.VITE_BACKEND_URL}/job/add`,
    jobData
  );
...
export const getJobPostingById = async (id: number) => {
  const response = await axios.get(
    `${import.meta.env.VITE_BACKEND_URL}/job/getById/${id}`
  );
  return response.data;
};
...
export const updateJobPosting = async (id: number, jobData: any) => {
  const response = await axios.put(
    `${import.meta.env.VITE_BACKEND_URL}/job/updateById/${id}`,
    jobData
  );
  return response.data;
};
...
export const deleteJobPosting = async (id: any) => {
  const response = await axios.delete(
    `${import.meta.env.VITE_BACKEND_URL}/job/deleteById/${id}`
  );
  return response.data;
};
